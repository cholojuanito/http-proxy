...
// A thread safe wrapper around Gethostbyname
struct hostent* getHostbyNameThreadSafe(const char *name) {
	sem_t mutex;
	Sem_init(&mutex, 0 , 1);
...
struct hostent *Gethostbyname(const char *name)
{
    ...
    if ((p = getHostbyNameThreadSafe(name)) == NULL)
	    dns_error("Gethostbyname error");
    return p;
}
/* $end gethostbyname */
...
int open_clientfd(char *hostname, char *port) {
    ...
    if ((rc = getaddrinfo(hostname, port, &hints, &results)) != 0) {
    ...
    if (p == NULL) { /* All connects failed */
        fprintf(stderr, "Could not connect to client or server\n");
        return -1;
    }
